...
public class PlayFabAdmin1 : MonoBehaviour
{
    ...
    private void OnListPlayersSuccess(ListPlayersResult result)
    {
        ...
        {
            startIndex += maxResults; // Cập nhật lại startIndex cho lần gọi tiếp theo
            GetPlayerList(); // Tiếp tục gọi API để lấy trang kết quả tiếp theo
        }
    }
    ...
    {
        Debug.LogError("Error retrieving player list: " + error.ErrorMessage);
    }
}
#endif
